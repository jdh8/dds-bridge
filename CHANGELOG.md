...
- `CurrentTrick` — a standalone type for a trick-in-progress carrying `trump`, `lead`, and 0–3 pairwise-distinct played cards. Constructable via `CurrentTrick::new(trump, lead)` or `CurrentTrick::from_slice(trump, lead, played)`, with incremental `try_push`. Shape and disjointness invariants are enforced once here and no longer re-checked by `Board::try_new`.
