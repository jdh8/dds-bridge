...
- `CurrentTrick` — a standalone type for a trick-in-progress carrying `trump`, `lead`, and 0–3 pairwise-distinct played cards. Constructable via `CurrentTrick::new(trump, lead)` or `CurrentTrick::from_slice(trump, lead, played)`, with incremental `try_push`. Exposes `winning_index` / `winning_card` / `winning_seat` (trump beats led suit beats off-suit, ties broken by rank) so the currently-winning play can be computed without a full `Board` or `PartialDeal`. Shape and disjointness invariants are enforced once here and no longer re-checked by `Board::with_trick`.
