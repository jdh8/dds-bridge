...
- **Breaking:** `Board` fields are now private. `Board::new(trump, lead)` always succeeds and initializes with empty remaining; use `Board::with_trick(trump, lead, remaining, played)` for a validated board. New accessors `trump()`, `lead()`, `current_cards()`, `remaining()`. The `remaining` field is now a `Subset` rather than a `Deal`. New `BoardError` enum covers invariant violations.
