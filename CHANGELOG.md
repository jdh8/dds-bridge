...
- **Breaking:** `Board` fields are now private. `Board::new` is removed; `Board::with_trick(trump, lead, remaining, played)` is the sole constructor (pass `&[]` for `played` at start of trick). New accessors `trump()`, `lead()`, `current_cards()`, `remaining()`. The `remaining` field is now a `PartialDeal` rather than a `Deal`. New `BoardError` enum covers invariant violations.
